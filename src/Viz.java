...
class View extends JPanel implements MouseListener
{
	...
	{
		state.positionOfPieces[rand.nextInt(22)] += (rand.nextInt(2) == 0 ? - 1 : 1);

		for (int i = 0; i < 11; i++)
		{
			System.out.print("(" + state.positionOfPieces[2 * i] + "," +
					state.positionOfPieces[2 * i + 1] + ") ");
		}
		System.out.println();
		viz.repaint();
	}
	                  ...
	                  int x1, int y1, int x2, int y2, int x3, int y3)
	{
		...
		b(state.positionOfPieces[2 * id] + x1, state.positionOfPieces[2 * id + 1] + y1);
		b(state.positionOfPieces[2 * id] + x2, state.positionOfPieces[2 * id + 1] + y2);
		b(state.positionOfPieces[2 * id] + x3, state.positionOfPieces[2 * id + 1] + y3);
	}
	...
	public void shape(int id, int red, int green, int blue, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4)
	{
		...
		b(state.positionOfPieces[2 * id] + x4, state.positionOfPieces[2 * id + 1] + y4);
	}
